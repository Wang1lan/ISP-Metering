...
% 正常 LUT 权重范围为 lambdaMin~lambdaMax，低计数时保留峰值分支基础贡献。
    ...
    names = {'fusionP1', 'fusionP2', 'lambdaMin', 'lambdaMax'};
    assert(all(isfield(cfg, names)), ...
        'buildFusionLUT:MissingConfig', ...
        'cfg 必须包含 fusionP1、fusionP2、lambdaMin 和 lambdaMax。');
    for k = 1:numel(names)
        value = cfg.(names{k});
    ...
    lambdaMin = double(cfg.lambdaMin);
    ...
    lutInfo = struct('validBlockCnt', Nvalid, 'C1', NaN, 'C2', NaN, ...
        'P1', P1, 'P2', P2, 'lambdaMin', lambdaMin, 'lambdaMax', lambdaMax);
    lambdaLUT = zeros(1, 0);
    if Nvalid > 0
        C1 = round(P1 * Nvalid);
        C2 = round(P2 * Nvalid);
        ...
        n = 0:Nvalid;
        lambdaLUT = lambdaMin * ones(1, Nvalid + 1);
        linearMask = n > C1 & n < C2;
        lambdaLUT(linearMask) = lambdaMin + (lambdaMax - lambdaMin) * ...
